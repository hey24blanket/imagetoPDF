...
function metadata(){document.documentElement.lang=lang;document.title=t.title;document.querySelector('meta[name=description]').content=t.description;document.querySelector('link[rel=canonical]').href=location.origin+'/'+lang+'/';document.querySelectorAll('[data-i18n]').forEach(e=>e.textContent=t[e.dataset.i18n]);document.querySelectorAll('[data-lang]').forEach(e=>{e.removeAttribute('aria-current');if(e.dataset.lang===lang)e.setAttribute('aria-current','true');});for(const [id,key]of [['zoom','zoom'],['prev','prev'],['next','next']]){$(id).setAttribute('aria-label',t[key]);$(id).title=t[key];}document.querySelector('.brand').href='/'+lang+'/';$('zoom-image').alt=t.preview;}
function setLanguage(next,remember=false){lang=next;t=messages[lang];if(remember){try{localStorage.setItem('print-language',lang);}catch{}document.cookie=`print-language=${lang};path=/;max-age=31536000;SameSite=Lax;Secure`;}history.replaceState(null,'','/'+lang+'/');metadata();renderCards();update();}
...
async function prepare(){if(cached)return cached;setBusy(true,t.generating);try{cached=await createPDF(items,settings(),(d,total)=>setBusy(true,`${t.generating} · ${d}/${total}`,d,total));return cached;}finally{setBusy(false);}}
...
function track(action){window.va?.('event',{name:'core_action',data:{action,app:'image-to-pdf',layout:count,language:lang}});}
